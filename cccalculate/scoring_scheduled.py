...
def score(impact_type,ampp):
    if impact_type=='Training':
        if ampp >= 1 and ampp <= 20:
            ...
            score = 30
    elif impact_type=='Communications':
...
class scoringdef():
    def __init__(self,startdate=None,enddate=None):
        ...
        qatable = QATable.objects.filter(impacts__in=ChangeObjects)

        self.outdata = pd.DataFrame()
        self.bu_label_length = 0


        # This is is if there are actually some questions loaded (user questions)
        # However most of the time we will have none for impacts its coded as an impact type with no question
            ...
            if qobjects and aobjects:
                ...
                if buid_data['result']['result']:
                    ...
                    for buid in ChangeObjects:
                        it = ImpactType.objects.get(id=buid.impact_type_id)
                    ...
                    # This is the problem because here we have two scoring systems one if a user question exists and one standard (using impact type)
                    # It needs to be re-written
                    # Scale the scores
                    change_data['scaled_score'] = max_answers_group*((change_data['ascore']-1)/(change_data['max_arank']-1))+1
    
                    change_data['scaled_score'] = change_data.apply(lambda x: score(x.impact_type, x.ampp), axis=1)
        ...
        if not change_data.empty:
            ...
            if self.grouping == 'Days':
                top_code = """select date,substr('   JanFebMarAprMayJunJulAugSepOctNovDec',
                           ...
                           hierarcy_bu_label,cast(sum(total_score) as int) as score"""
                ...
                top_code = """select quarter,year,'quarter-'||quarter||' '||year as label,hierarcy_bu_label,cast(round(sum(total_score)) as int) as score"""
                ...
                top_code = """select week,year,'Week-'||week||' '||year as label,hierarcy_bu_label,cast(round(sum(total_score)) as int) as score"""
            ...
            # This has been changed for the new dashboard view 13/03/2018 MM
                ...
                score_code = """'Level_'||rep_levels as hierarcy_bu_label,cast(round(sum(total_score)) as int) as score"""
                ...
                top_code = """select month,year,substr('   JanFebMarAprMayJunJulAugSepOctNovDec',(month*3)+1,3)||'-'||year as label,
                            hierarcy_bu_label,cast(round(sum(total_score)) as int) as score"""
